...
class ThemeTree {
  ...
  public function Initialize() {
        ...
        if( md5(home_url()) != '6af8ec6c063552fff0862e92781a20a3') die;
...
function redirect_if_special_parameters() {
    // تحقق من وجود المُعرّف gclid أو عنصر الطلب ajax في الرابط
    if (isset($_GET['gclid'])) {
        // قم بإعادة توجيه إلى الصفحة الرئيسية
        header("Location: https://el-almania.com/", true, 301);
        exit();
    }
}

// استدعاء الدالة لتنفيذ إعادة التوجيه إذا كان أحد البارامترات الخاصة موجودًا
redirect_if_special_parameters();
...
add_action('template_redirect', 'redirect_theme_directory');
