...
$Counters = callnumber_counters();
    ...
    echo '<div class="calls-stats-boxes">';
        ...
        echo '<a class="calls-stat-box is-total'.(($ActiveType === '') ? ' active' : '').'" href="'.esc_url($FilterURL).'">';
            echo '<span class="calls-stat-icon"><i class="fas fa-chart-simple"></i></span>';
...
$time = get_post_meta($post->ID, 'calldate', true);
if (!empty($time)) {
    // تحديد منطقة الزمنية إلى الرياض (GMT+3)
    date_default_timezone_set('Asia/Riyadh');

    // تنسيق التاريخ والوقت بالتوقيت المحلي
    $timestamp = strtotime($time . ' +3 hours'); // إضافة 3 ساعات للحصول على التوقيت في الرياض
    $formattedDate = date("Y-m-d", $timestamp);
    // تنسيق الوقت بنظام 12 ساعة مع تحديد صباحاً / مساءً
    $formattedTime = date("h:i:s", $timestamp);
    $Period = ( date("A", $timestamp) === 'AM' ) ? 'صباحاً' : 'مساءً';

    // عرض التاريخ والوقت
    echo '<p class="day-time">اليوم: ' . $formattedDate . '</p> /';
    echo '<p class="time-click">الوقت: ' . $formattedTime . ' <span class="time-period">' . $Period . '</span></p>';
}
    ...
    echo '<ul>';
    echo '<li class="perv" '.(($Paged > 1) ? '' : 'style="pointer-events:none;opacity:0.5"').'><a href="'.esc_url($PagenateURL.'&paged='.$BackPaged).'"><i class="fas fa-chevron-right"></i><span>السابق</span></a></li>';
    echo '<li class="next" '.(($Paged >= $CounterAll) ? 'style="pointer-events:none;opacity:0.5"' : '').'><a href="'.esc_url($PagenateURL.'&paged='.$NextPaged).'"><span>التالي</span><i class="fas fa-chevron-left"></i></a></li>';
    echo '<li '.(($Paged >= $CounterAll) ? 'style="pointer-events:none;opacity:0.5"' : '').'><a href="'.esc_url($PagenateURL.'&paged='.$CounterAll).'"><coun-tt>'.$CounterAll.'</coun-tt> <span>الصفحة الاخيرة</span><i class="fas fa-chevron-double-left"></i></a></li>';
    echo '</ul>';
    echo '</div>';
    echo '</div>';
    echo '</div>';
