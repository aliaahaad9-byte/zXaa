...
// تتبع نقرات الاتصال المباشر والواتساب مع نوع النقرة ورابط الصفحة
var YCLastTrack = {};
function YCTrackCall(type) {
    var now = Date.now();
    if (YCLastTrack[type] && now - YCLastTrack[type] < 3000) {
        return; // منع التسجيل المكرر خلال 3 ثوانٍ
    }
    YCLastTrack[type] = now;
    $.ajax({
...
$("body").on("click", 'a[href*="wa.me/"], a[href*="api.whatsapp.com"], a[href*="whatsapp://"]', function() {
    YCTrackCall("whatsapp");
});