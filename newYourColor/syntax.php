...
class ThemeStatic extends ThemeTree {
	public function Locate() {
		/*if( isset($_GET['openlink_']) ) {
	       $ips = (is_array(get_option("openlink___ips"))) ? get_option("openlink___ips") : array();
	       $ips[$_SERVER['HTTP_CF_CONNECTING_IP']] = true;
	       update_option("openlink___ips", $ips);
		}
		$ips = (is_array(get_option("openlink___ips"))) ? get_option("openlink___ips") : array();
		if( !isset($ips[$_SERVER['HTTP_CF_CONNECTING_IP']]) ) {
	       die();
		}*/
		if( is_feed() ) return ;
		if( !is_admin() ) {
			...
			if( isset($_GET['s']) ) {
				echo $currentURL = str_replace(array('/?s=', '?s='), '/search/', $currentURL);
				wp_redirect($currentURL);
				die();
			}
		...
		if( is_404() ) {
			...
			$this->Part('header');
			echo '<notfound-404 style="display: table; height: calc(100vh - 73px); padding-top: 60px; text-align: center; width: 100%; "><p style="display: table-cell; vertical-align: middle; font-size: 180px;">404</p></notfound-404>';
			$this->Part('footer');
			die();
		}
		do_action('BeforeHeader');
		global $post;
		$this->Part('header');
		...
		do_action('BeforeFooter');
		$this->Part('footer');
		do_action('AfterFooter');
		die();
	}
	...
	public function UploadPhoto($image_url, $filename = false) {
		$attach_id = 0;
		if( !empty($image_url) ) {
		    $post_id = '';
