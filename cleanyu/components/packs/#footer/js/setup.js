...
/* ============================================================
   تسجيل المكالمات : يتم إرسال نوع الاتصال ( هاتفي / واتساب )
   مع اسم الصفحة إلى صفحة المكالمات في لوحة التحكم
   ============================================================ */
var CallTrackLocked = false;

function CallTrackType(el) {
    var $el = jQuery(el);
    var href = String($el.attr("href") || $el.closest("a").attr("href") || "");
    var data = String($el.attr("data-call") || $el.closest("[data-call]").attr("data-call") || "").toLowerCase();

    if (data === "whatsapp" || $el.closest(".btn-whatsapp, .popup-whatsapp, .ads-whatsapp-box").length || /wa\.me|whatsapp/i.test(href)) {
        return "whatsapp";
    }
    if (data === "phone" || $el.closest(".btn-phone, .popup-phone, .ads-phone-box, .price_Alniks").length || /^tel:/i.test(href)) {
        return "phone";
    }
    return false;
}

$("body").on("click", '.btn-phone, .btn-whatsapp, .popup-phone, .popup-whatsapp, .ads-phone-box, .ads-whatsapp-box, [data-call], a[href^="tel:"], a[href*="wa.me"], a[href*="api.whatsapp.com"], a[href*="web.whatsapp.com"]', function() {

    if (CallTrackLocked) {
        return;
    }

    var CallType = CallTrackType(this);
    if (CallType === false) {
        return;
    }

    CallTrackLocked = true;
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            CallTrackLocked = false; // Reset after success
        },
        error: function(xhr, status, error) {
            console.error("Error: " + error);
            CallTrackLocked = false; // Reset after error
        }
    });
});
