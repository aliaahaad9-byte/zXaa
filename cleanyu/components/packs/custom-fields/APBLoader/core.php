...
class APBFields extends APB {
	# Explicit property declarations to fix PHP 8.2 dynamic property deprecation
	public  $args = [];
	public  $boxes = [];
	public  $layouts = [];
	public $type = 'add';
	public $post = 0;  # post ID or term ID
	...
	public function __construct(array $arguments = []) {
		...
		if (isset($Params['action']) && $Params['action'] === 'edit' && isset($_GET['post'])) {
			$this->type = 'edit';
			$this->post = (int) $Params['post'];
		} elseif (isset($Params['taxonomy']) && isset($Params['tag_ID']) && is_numeric($Params['tag_ID'])) {
			$this->type = 'edit';
			$this->post = (int) $Params['tag_ID'];
		} else {
			$this->type = 'add';
			$this->post = 0;
		}
	...
	public function AjaxFields($layout, $fields, $numb) {
	...
	public function APBAddLayoutBuilder($metabox, $numb) {
		if( $metabox && isset( $this->boxes[$metabox] ) ){
	...
	public function SetupFields($args, $MetaboxID, $numb, $taxonomy=false) {
				...
				$numb = 0; foreach ($value as $k2 => $v) {
					if( isset($value[$k2]) ) {
						echo '<div class="APBMainLayout" data-numb="'.$numb.'">';
							...
							echo '<div class="APBLayoutsBuilder">';
								if( !empty($value) ) {
	...
	public function FieldType($field, $hasparent, $addmore, $numb, $metaboxid, $layout, $taxonomy=false) {
		if( $field['type'] == 'title' ) {
			$name = $field['id'];
			$id = $field['id'];
