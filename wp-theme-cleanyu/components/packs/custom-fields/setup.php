...
class APB {
    ...
    # Explicit properties to avoid dynamic property deprecation
    private $layouts = [];
    private $type = 'add';
    private $post = 0;
	...
	public function SavePost($postID) {
		...
		if( $this->CanSave() and isset($this->Methods()['apbupdate']) ) {
			...
			if($MyPost->post_type == 'academy'){

				$CoursesTime = (new CoursesCanter)->UpdateRunTime($MyPost->ID);

			}

			#

			if( $MyPost->post_type == 'webinars' && isset( $this->Methods()['starts_date'] ) || $MyPost->post_type == 'seminars' && isset( $this->Methods()['starts_date'] ) ){

				$StrToTime = strtotime( $this->Methods()['starts_date'] );

				#

				$this->UpdatePost($postID, 'str_starts_date' , $StrToTime );

			}else{

				$this->RemovePost($postID, 'str_starts_date' );
